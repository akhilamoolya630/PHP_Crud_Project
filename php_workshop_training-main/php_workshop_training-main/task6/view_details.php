...
<form action="view_details.php" method="get">
    ...
    Sort by: <input type="text" name="sort" value="<?php echo isset($_GET['sort']) ? $_GET['sort'] : ''; ?>">
...
<table border="1">
    ...
    $sort_by = "name";
    if (isset($_GET['sort']) && in_array($_GET['sort'], ['name', 'usn', 'phone'])) {
        $sort_by = $_GET['sort'];
    }

    // Prepare the SQL query with search and sort
